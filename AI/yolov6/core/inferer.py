            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    if save_img:
                        ...
                        # cropped_plate = img_ori[int(xyxy[1]):int(xyxy[3]), int(xyxy[0]):int(xyxy[2])]
                ...
                if save_img:
                    failed_save = osp.join(save_dir, rel_path, 'failed', osp.basename(img_path))
                    LOGGER.info(f'No detections found for {failed_save}')
                    cv2.imwrite(failed_save, img_src)
            # FPS counter
